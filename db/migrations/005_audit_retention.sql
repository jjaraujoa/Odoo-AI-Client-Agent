...
DROP FUNCTION IF EXISTS agent.cleanup_expired_data();

CREATE FUNCTION agent.cleanup_expired_data()
RETURNS TABLE(
    memory_deleted bigint,
    sessions_cleared bigint,
    actions_expired bigint,
...
BEGIN
    ...
    DELETE FROM agent.audit_events WHERE created_at < now() - interval '30 days';
    GET DIAGNOSTICS v_audit = ROW_COUNT;

    RETURN QUERY SELECT v_memory, v_sessions, v_actions, v_rate, v_audit;
END;
$$;

INSERT INTO agent.schema_migrations(version) VALUES ('005_audit_retention')
